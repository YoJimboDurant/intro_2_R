...
books available online at Books 24/7 through HHS university (has approx. 27 books and 30 short videos). Massive Online Courses
(MOC's) such as coursera offer good instruction. If you are interested in 
using R to learn R - swirl is an excellent way to provide interactive lessons that 
teach you R as you use R. The RUG also has several of its past presentation materials and video's archived and available for users. Free online resources such as R for Data Science by Garrett Grolemund and Hadley Wickham are excellent intermediate/beginer texts.
...
- R uses a floating point number
- "e" and 
...
```
<article>
Floating point arithmatic and issues are tricky to grasp. Chapter 1 of the R Inferno by Burns has a good discussion http://www.burns-stat.com/pages/Tutor/R_inferno.pdf
<\article>
...
## Significant Digits
 ...
 - By default R will print a set number of digits (`getOption("digits")`) to see
...
hist(randomNums, main = "Histogram", col="lightblue") 
plot(density(randomNums), main = "Density")
par(oldpar)
```
...
```{r dfx}
 AGE_Surveillance_Log <- read_excel(
   "../data/AGE Surveillance Log.xls", 
   skip = 7)
names(AGE_Surveillance_Log)[
